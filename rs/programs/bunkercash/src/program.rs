...
/// Recovery admin key (devnet only): allows migrating `pool.admin` if it was set to
/// an unreachable/incorrect address during governance setup.
/// This is the public key for the deploy wallet at `~/.config/solana/id.json`.
pub const RECOVERY_ADMIN_PUBKEY: Pubkey = pubkey!("3BXEsRgUmrTudbZDzQjDpA2mvwV7vDC73WGjhHPRGBee");
...
pub struct UpdateAdmin<'info> {
    #[account(
        ...
        // Recovery: allow the deploy wallet to migrate admin during setup.
        constraint = (admin.key() == pool.admin)
            || (pool.admin == SQUADS_MULTISIG_PUBKEY && admin.key() == SQUADS_VAULT_PUBKEY)
            || (admin.key() == RECOVERY_ADMIN_PUBKEY)
            @ ErrorCode::Unauthorized
    )]
    pub pool: Account<'info, PoolState>,
    /// Current admin signer (wallet pre-governance, Squads vault post-governance).
